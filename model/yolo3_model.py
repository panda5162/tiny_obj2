...
class gan_model:
    ...
    def _d_block(self, inputs, filters_num, out_filters, conv_index, training = True, norm_decay = 0.99, norm_epsilon = 1e-3):
        """
        Introduction
        ------------
    ...
    def d_inference(self, inputs, num_anchors, num_classes, training = True):
        """
        Introduction
        ------------
            构建yolo模型结构
        ...
        with tf.variable_scope('yolo'):
            ...
            conv2d_57, conv2d_59, conv_index = self._d_block(conv, 512, num_anchors * (num_classes + 5), conv_index = conv_index, training = training, norm_decay = self.norm_decay, norm_epsilon = self.norm_epsilon)
            ...
            conv2d_65, conv2d_67, conv_index = self._d_block(route0, 256, num_anchors * (num_classes + 5), conv_index = conv_index, training = training, norm_decay = self.norm_decay, norm_epsilon = self.norm_epsilon)
            ...
            _, conv2d_75, _ = self._d_block(route1, 128, num_anchors * (num_classes + 5), conv_index = conv_index, training = training, norm_decay = self.norm_decay, norm_epsilon = self.norm_epsilon)

        return [conv2d_59, conv2d_67, conv2d_75, out1]



    def d_head(self, feats, anchors, num_classes, input_shape, training = True):
        """
        Introduction
        ------------
    ...
    def d_boxes_scores(self, feats, anchors, num_classes, input_shape, image_shape):
        """
        Introduction
        ------------
        ...
        box_xy, box_wh, box_confidence, box_class_probs = self.d_head(feats, anchors, num_classes, input_shape, training = False)
        box_yx = box_xy[..., ::-1]
        box_hw = box_wh[..., ::-1]
    ...
    def yolo_loss(self, yolo_output, y_true, anchors, num_classes, tf_mask, ignore_thresh = .5):
        """
        Introduction
        ------------
            yolo模型的损失函数
        ...
        for index in range(1):   #33333333333333~~~~
            ...
            grid, predictions, pred_xy, pred_wh = self.d_head(yolo_output[index], anchors[anchor_mask[index]], num_classes, input_shape, training = True)
            ...
            xy_loss = object_mask * box_loss_scale * tf.square(raw_true_xy - predictions[..., 0:2])
            wh_loss = object_mask * box_loss_scale * 0.5 * tf.square(raw_true_wh - predictions[..., 2:4])
            confidence_loss = object_mask * tf.square(object_mask - predictions[..., 4:5]) + (1 - object_mask) * tf.square(object_mask - predictions[..., 4:5]) * ignore_mask
            class_loss = object_mask * tf.square(class_probs - predictions[..., 5:])
        ...
        if tf_mask == 1:
            d_loss1 = tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(yolo_output[3]), logits=yolo_output[3])
        if tf_mask == 0:
            d_loss1 = tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(yolo_output[3]), logits=yolo_output[3])
