...
def distort_color(image, color_ordering=0, fast_mode=True, scope=None):
  ...
  with tf.name_scope(scope, 'distort_color', [image]):
    ...
    return tf.clip_by_value(image, 0.0, 1.0)  # clip_value_min：需要过滤的最小值，若t中存在比该值还小的值，一律换成clip_value_min
...
def ssd_random_sample_patch(image, labels, bboxes, ratio_list=[0.1, 0.3, 0.5, 0.7, 0.9, 1.], name=None):
  ...
  def check_roi_center(width, height, labels, bboxes):
    with tf.name_scope('check_roi_center'):
      ...
      mask_labels = tf.boolean_mask(labels, mask)  #tf.boolean_mask(a,b) 将使a (m维)矩阵仅保留与b中“True”元素同下标的部分，并将结果展开到m-1维。
      mask_bboxes = tf.boolean_mask(bboxes, mask)  #https://www.cnblogs.com/lyc-seu/p/7956231.html
