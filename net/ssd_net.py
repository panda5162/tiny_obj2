...
class ReLuLayer(tf.layers.Layer):
    def __init__(self, name, **kwargs):
        super(ReLuLayer, self).__init__(name=name, trainable=trainable, **kwargs)  #?????trainable
        self._name = name
    def build(self, input_shape):
        self. relu = lambda x : tf.nn.relu(x, name=self._name)
        self.built = True
...
class VGG16Backbone(object):
    def __init__(self, data_format='channels_first'):
        ...
        self._conv_initializer = tf.glorot_uniform_initializer  #由一个均匀分布（uniform distribution)来初始化数据
    ...
    def forward(self, inputs, training=False):
        ...
        for conv in self._conv4_block:
            inputs = forward_module(conv, inputs, training=training)
        # conv4_3
        with tf.variable_scope('conv4_3_scale') as scope:  #对conv4_3进行norm操作
