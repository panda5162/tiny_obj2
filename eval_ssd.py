...
def ssd_model_fn(features, labels, mode, params):
    ...
    # pree = list(predictions)


    summary_dir1 = os.path.join(FLAGS.model_dir, 'predict111')
    for class_ind in range(1, FLAGS.num_classes):
        with open(os.path.join(summary_dir1, 'results_{}.txt'.format(class_ind)), 'wt') as f:
            for image_ind, pred in enumerate(predictions):
                # print('qwertyuiop')
                # print(pred)
                # print(pred[0:8])
                # print('qwertyuiop')
