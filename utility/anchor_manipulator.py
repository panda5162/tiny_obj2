...
def iou_matrix(gt_bboxes, default_bboxes):
    with tf.name_scope('iou_matrix', [gt_bboxes, default_bboxes]):
        ...
        return tf.where(tf.equal(union_vol, 0.0),
                        tf.zeros_like(inter_vol), tf.truediv(inter_vol, union_vol))  #对应位置元素的除法运算。x,y具有相同尺寸的tensor，x为被除数，y为除数。
...
def do_dual_max_match(overlap_matrix, low_thres, high_thres, ignore_between=True, gt_max_first=True):
    ...
    with tf.name_scope('dual_max_match', [overlap_matrix]):
        ...
        less_mask = tf.less(match_values, low_thres)  #0.5
        ...
        #one_hot将一个值化为一个概率分布的向量，一般用于分类问题。
...
#allowed_borders = [1.0] * 6, positive_threshold = FLAGS.match_threshold(0.5), ignore_threshold = FLAGS.neg_threshold(0.5), prior_scaling=[0.1, 0.1, 0.2, 0.2])
    ...
    def encode_all_anchors(self, labels, bboxes, all_anchors, all_num_anchors_depth, all_num_anchors_spatial, debug=False):
        ...
        with tf.name_scope('encode_all_anchors'):
            ...
            for ind, anchor in enumerate(all_anchors):
                ...
                list_anchors_ymin.append(tf.reshape(anchors_ymin_, [-1]))  #一维
            ...
            anchors_ymin = tf.concat(list_anchors_ymin, 0, name='concat_ymin')  #连接两个矩阵的操作
            ...
            if self._clip:
                anchors_ymin = tf.clip_by_value(anchors_ymin, 0., 1.)  #当v小于a时，输出a； 当v大于a小于b时，输出原值；当v大于b时，输出b；
            ...
            anchor_allowed_borders = tf.stack(tiled_allowed_borders, 0, name='concat_allowed_borders')  #矩阵拼接
...
class AnchorCreator(object):
    ...
    def get_layer_anchors(self, layer_shape, anchor_scale, extra_anchor_scale, anchor_ratio, layer_step, offset = 0.5):
        ...
        with tf.name_scope('get_layer_anchors'):
            x_on_layer, y_on_layer = tf.meshgrid(tf.range(layer_shape[1]), tf.range(layer_shape[0]))  #生成网络
            ...
            num_anchors_along_depth = len(anchor_scale) * len(anchor_ratio) + len(extra_anchor_scale)  #4,6,6,6,4,4
            ...
            for scale_index, scale in enumerate(anchor_scale): #0.1
                for ratio_index, ratio in enumerate(anchor_ratio): #1, 2, 0.5
                    list_h_on_image.append(scale / math.sqrt(ratio))  #平方根
...
#-1表示最后一维，添加1
    ...
    def get_all_anchors(self):
        ...
        for layer_index, layer_shape in enumerate(self._layers_shapes):
            ...
            all_anchors.append(anchors_this_layer[:-2]) #输出倒数第二行之前的 长度为6(6次循环）
            all_num_anchors_depth.append(anchors_this_layer[-2])
            all_num_anchors_spatial.append(anchors_this_layer[-1])  #倒数第一
